...
const steps = [
  {
    ...
    content:
      'Vamos fazer um tour rápido para você conhecer as principais funcionalidades da sua área logada.',
  },
  {
    target: '[data-tour="menu-coleta"]',
    title: 'Agendar Coleta',
    content:
      'Aqui você pode solicitar novas coletas e acompanhar o status das suas solicitações ativas.',
  },
  {
    target: '[data-tour="menu-cotacao"]',
    title: 'Cotação Rápida',
    content: 'Faça simulações de frete instantâneas informando origem, destino e dados da carga.',
  },
  {
    target: '[data-tour="menu-docs"]',
    title: 'Documentos',
    content: 'Baixe CTe, faturas, boletos e outros documentos importantes com apenas um clique.',
  },
]

export function PortalTour() {
  const { hasSeenTour, setHasSeenTour } = usePortalStore()
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100000] pointer-events-none">
      {rect ? (
        <div
          className="absolute border-2 border-[#800020] rounded-lg transition-all duration-300 ease-in-out pointer-events-none"
          style={{
            top: rect.top - 4,
            left: rect.left - 4,
            width: rect.width + 8,
      ...
      >
        <Card className="w-[320px] shadow-2xl border-[#800020]/20 bg-white animate-in zoom-in-95">
          <CardHeader className="pb-3 border-b border-slate-100 bg-slate-50 rounded-t-lg relative">
            ...
            <CardTitle className="text-sm font-black flex items-center gap-2 text-[#800020]">
              <div className="bg-[#800020]/10 p-1.5 rounded-md">
          ...
          <CardFooter className="flex items-center justify-between border-t border-slate-100 pt-3 pb-3 bg-slate-50 rounded-b-lg">
            <div className="flex gap-1.5">
              {steps.map((_, i) => (
                <div
                  key={i}
                  className={`w-1.5 h-1.5 rounded-full transition-colors ${i === currentStep ? 'bg-[#800020] w-3' : 'bg-slate-300'}`}
                />
              ))}
            </div>
            <div className="flex gap-2">
              ...
              <Button
                size="sm"
                onClick={handleNext}
                className="h-7 text-xs px-3 bg-[#800020] hover:bg-[#5c0017] text-white shadow-sm font-bold"
              >
